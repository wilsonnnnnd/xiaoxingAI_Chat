...
### 🤖 智能对话系统
- **自然语言处理**：基于 Qwen1.5-7B 大语言模型，支持流畅的中文对话
- **上下文理解**：记住对话历史，支持多轮连续对话
- **个性化回应**：根据用户特点调整回复风格和内容
- **智能问答**：涵盖日常生活、知识问答、情感支持等多个领域

### 🧠 记忆与学习系统
- **长期记忆**：永久保存用户偏好、重要信息和聊天历史
- **智能标签**：自动为对话内容添加话题和情绪标签
- **偏好学习**：学习用户的兴趣爱好、生活习惯和沟通风格
- **记忆检索**：能够回忆起之前的对话内容和重要信息

### 😊 情绪识别与回应
- **情绪分析**：实时分析用户的情绪状态（积极、消极、中性）
- **情感词典**：内置丰富的情感词汇库，准确识别情绪表达
- **情绪回应**：根据用户情绪提供相应的安慰、鼓励或陪伴
- **情绪统计**：跟踪和分析用户的情绪变化趋势

### 🎵 多模态语音系统
- **语音合成**：使用 Microsoft Edge TTS，支持多种中文语音
- **情绪语音**：根据对话情绪调整语音风格、语速和音调
- **个性化配置**：可自定义语音角色、语速、音量等参数
- **语音日志**：记录语音合成历史，支持回放和管理

### 📊 对话分析与总结
- **自动总结**：定期生成对话摘要和主题分析
- **数据可视化**：生成情绪图表、词云等可视化报告
- **定时任务**：支持每日自动总结和报告生成
- **趋势分析**：分析聊天频率、情绪变化等长期趋势

### 💾 数据管理系统
- **SQLite 数据库**：可靠的本地数据存储，支持复杂查询
- **数据备份**：支持数据导出和备份恢复
- **隐私保护**：所有数据本地存储，不上传云端
- **数据清理**：提供数据清理和优化工具
...
xiaoxing/                               # 项目根目录
├── 📁 .bat/                            # 批处理启动脚本
│   ├── 小星PyAPI.bat                    # Python API 模式启动（推荐）
│   ├── 启动小星_API模式.bat              # llama.cpp API 模式启动
│   ├── 小星快速启动.bat                 # 快速启动脚本（命令行模式）
│   ├── 构建小星.bat                    # 构建 llama.cpp 并启动
│   └── test.bat                       # API 连接测试脚本
├── 📁 config/                          # 配置文件目录
│   ├── config.py                      # 🔧 主配置文件（API地址、参数等）
│   └── prompt.txt                     # 🎭 系统提示词（小星的人格设定）
├── 📁 db/                             # 数据库相关文件
│   ├── init_db.py                     # 📊 数据库初始化脚本
│   ├── schema_output.sql              # 🏗️ 数据库结构定义文件
...
│   │   └── stopwords_db.py            # 停用词数据库管理
│   ├── 📁 emotion/                    # 😊 情绪识别模块
│   │   ├── emotion_dict_db.py         # 情绪词典数据库操作
│   │   └── emotion_utils.py           # 🧠 情绪分析和识别工具
│   ├── 📁 log/                        # 📝 日志记录模块
│   │   └── chat_logger.py             # 聊天对话日志记录器
│   ├── 📁 memory/                     # 🧠 记忆系统模块
│   │   ├── memory.py                  # 💭 核心记忆类
│   │   ├── memory_tools.py            # 🔍 记忆工具函数
│   │   ├── memory_parser.py           # 📖 记忆内容解析器
│   │   └── preference_db.py           # ❤️ 用户偏好数据库操作
│   └── 📁 summary/                    # 📊 对话总结模块
│       ├── scheduler_manager.py       # ⏰ 定时任务管理器
│       ├── summary_db.py              # 📈 总结数据库操作
│       ├── summary_manager.py         # � 对话总结管理器
│       └── summary_reporter.py        # 📊 总结报告生成器
├── �📁 llama.cpp/                      # LLaMA 模型引擎目录
│   ├── 📁 build/                      # 🔨 编译输出目录
│   │   └── 📁 bin/                    # 可执行文件目录
│   │       ├── llama-cli.exe          # 命令行推理程序
│   │       └── llama-server.exe       # API服务器程序
│   ├── 📁 models/                     # 🤖 模型文件存储目录
│   │   └── qwen1_5-7b-chat-q5_k_m.gguf # AI模型文件（需下载）
│   └── ...                           # llama.cpp 源码和其他文件
├── 📁 memory/                         # 记忆数据存储目录
│   ├── 📁 audio/                      # 🎵 语音文件存储
│   ├── 📁 report/                     # 📊 生成的报告文件
...
├── 📁 ui/                             # 用户界面相关
│   ├── viewer.html                    # 📱 网页查看器
│   └── visualizer_ui.py               # 🖼️ 数据可视化界面
├── main_chat.py                       # 🚀 主程序入口文件
├── keep_awake.py                      # 💻 防休眠工具
...
### 数据存储位置
- **数据库文件**：`E:\xiaoxing\db\xiaoxing_memory.db`
...
建议定期备份数据库文件：
```bash
copy E:\xiaoxing\db\xiaoxing_memory.db E:\backup\xiaoxing_backup_20241015.db
```
